<?php 
    ...
    // Connect to the DB
    $db = mysqli_connect("localhost", "root", "mysql", "brainwhiz");
    ...
    if (isset($_POST['upload'])){
        ...
        shell_exec("/Users/alex/anaconda3/bin/python centerslice.py $nifti_path $x_path $y_path $z_path");
...
<body>
    ...
    if (isset($_POST['upload']) and (!empty($_FILES['nifti']['tmp_name'])))
    {
        if (mysqli_query($db, $sql) )
        {
            echo "<h1> sucessfuly inserted in DB </h1>";
            
        }
        else 
        {
            echo "<h1> failed to insert in DB </h1>";
        }
